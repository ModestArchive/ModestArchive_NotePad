...
FindPreviousCommand::FindPreviousCommand(NotePadFormControlWnd* notePadFormControlWnd, NotePadForm* notePadForm) :Command(notePadForm) {
...
void FindPreviousCommand::Execute() {
	if (this->notePadFormControlWnd->GetContentsFinder() == 0) {//처음으로 찾기기능을 사용했을때
		...
		if (this->notePadForm->GetIsSelected() == true) {
			...
			cFindReplaceDialog->Create(true, _T(""), _T(""), FR_NOWHOLEWORD, ((CWnd*)(this->notePadFormControlWnd)));
		}
		...
		if (*contentsFinder->GetFindWhatString() == "\0") {
			...
			findReplaceSupervisor.GetReadyToFindPreviousFromMenuCommand();

			findReplaceSupervisor.DoSearchUpForMenuCommand();
		}
	}
}