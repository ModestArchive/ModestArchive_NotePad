...
CutSelectionCommand::CutSelectionCommand(NotePadFormControlWnd* notePadFormControlWnd, NotePadForm* notePadForm) :Command(notePadForm) {
...
void CutSelectionCommand::Execute() {
	if (this->notePadForm->GetIsSelected() == true) {
		...
		Selector* newSelector = new Selector(this->notePadForm);
		this->notePadForm->SetSelector(newSelector);
	}
}